...
class Db2QtHelp:
    ...
    def _write_sections_recursive(self, c : str, dst_folder : str, pages, files, app_name : str, fdo_content : io.TextIOWrapper, level : int) -> None:
        ...
        # patch links (were links to anchors, now they get proper links to split pages
        ...
        if len(subs)>1:
            ...
            for i,sub in enumerate(subs):
                ...
                #for s in src_folder:
                #    sub = sub.replace(f"src=\"{s}/", f"src=\"qthelp://{app_name}/doc/")
                self._write_sections_recursive(sub, dst_folder, pages, files, app_name, fdo_content, level+1)
            #fdo_content.write(indent + "</ul>\n")
        #self._toc += indent + "</section>\n"


    def _process_single(self, source : str, dst_folder : str, pages, files, app_name : str) -> None:
        ...
        with open(f"{dst_folder}/toc.html", "w", encoding="utf-8") as fdo_content:
            ...
            for c in chapters:
                self._write_sections_recursive(c, dst_folder, pages, files, app_name, fdo_content, 1)
    ...
    def _process_chunked(self, folder : str, pages, files, app_name : str) -> None:
    ...
    def build_keywords(self, pages):
        keywords = ""
        for page in pages:
            filename = page[0]
            title = page[1]
            keywords += f"   <keyword name=\"{title}\" ref=\"./{filename}\"/>\n"
        return keywords
    ...
    def process(self, source : str, dst_folder : str, app_name : str) -> None:
        ...
        if os.path.isdir(source):
            ...
            self._process_chunked(source, pages, files, app_name)
            ...
            if source.endswith(".html"):
                ...
                self._process_chunked(tmp_dir, pages, files, app_name)
            else:
        ...
        for file in files:
            _, filename = os.path.split(file)
            shutil.copy(file, f"{dst_folder}/{filename}")
        # sort pages
        ...
        keywords = self.build_keywords(pages)
