...
class Db2QtHelp:
    ...
    def _write_sections_recursive(self, c : str, src_folder : List[str], dst_folder : str, app_name : str, fdo_content : io.TextIOWrapper, level : int) -> None:
        ...
        Args:
            ...
            src_folder (List[str]): List of folders to copy files from
        ...
        if len(subs)>1:
            ...
            for i,sub in enumerate(subs):
                ...
                self._write_sections_recursive(sub, src_folder, dst_folder, app_name, fdo_content, level+1)
            fdo_content.write(indent + "</ul>\n")
        self._toc += indent + "</section>\n"


    def _process_single(self, source : str, src_folder : List[str], dst_folder : str, app_name : str) -> None:
        ...
        with open(f"{dst_folder}/toc.html", "w", encoding="utf-8") as fdo_content:
            ...
            for c in chapters:
                self._write_sections_recursive(c, src_folder, dst_folder, app_name, fdo_content, 1)
    ...
    def process(self, source : str, src_folder : List[str], dst_folder : str, app_name : str) -> None:
        ...
        for s in src_folder:
            ...
            for f in files:
                _, n = os.path.split(f)
                shutil.copy(f, f"{dst_folder}/{n}")
        # process
        if os.path.isdir(source):
            print(f"Processing chunked HTML output from '{source}'")
            self._process_chunked(source)
        elif os.path.isfile(source):
            if source.endswith(".html"):
                print(f"Processing single HTML output from '{source}'")
                self._process_single(source, src_folder, dst_folder, app_name)
        ...
        with open(path + ".qhp", "w", encoding="utf-8") as fdo:
            fdo.write(self._qhp_template.replace("%toc%", self._toc).replace("%keywords%", self._keywords).replace("%appname%", app_name))
        # generate qhcp
